...
class OperatingUnit(models.Model):
    ...
    def write(self, vals):
        ...
        # pre-existing rows (e.g. the demo "Main Operating Unit") already
        # have it empty, so the DB-level constraint alone cannot be trusted
        # to enforce this either way.
    ...
    def _check_analytic_account_id(self, analytic_account_id):
        if not analytic_account_id:
            raise exceptions.ValidationError(
                self.env._("The analytic account is required for every operating unit.")
            )
